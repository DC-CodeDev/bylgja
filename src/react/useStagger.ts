...
export function linearStagger(index: number, count: number): number {
  if (count <= 1) {
    return 0;
  }
...
export function centerOutStagger(index: number, count: number): number {
  if (count <= 1) {
    return 0;
  }
...
export function useStagger(options: UseStaggerOptions): StaggerBinding[] {
  const {
    count,
    curve = linearStagger,
...
function validateStaggerDelay(staggerDelay: number): void {
...
function validateFactor(factor: number, index: number, count: number): void {
