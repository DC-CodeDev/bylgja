...
export function createWavePropagation(elements, origin, config) {
    if (elements.length === 0) {
        throw new Error("createWavePropagation requires at least one element to compute propagation.");
    }
    ...
    if (config?.propagationSpeed !== undefined) {
        if (!Number.isFinite(propagationSpeed) || propagationSpeed <= 0) {
            throw new Error("createWavePropagation requires propagationSpeed to be a finite number greater than zero.");
        }
    }
    if (config?.minAmplitude !== undefined || config?.maxAmplitude !== undefined) {
        ...
        if (minAmplitude > maxAmplitude) {
            throw new Error("createWavePropagation requires minAmplitude to be less than or equal to maxAmplitude.");
        }
    }
...
//# sourceMappingURL=wave-propagation.js.map